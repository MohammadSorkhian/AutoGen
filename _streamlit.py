    ...
    def _track_response_on_streamlit(self, msg):
        if isinstance(msg, ToolCallRequestEvent):
            content = f"**[{msg.source}] Tool calls requested:**\n- " + "\n- ".join(
                f"`{tool.name}` with arguments `{tool.arguments})`"
...
with st.sidebar:
    ...
    model = st.selectbox(
        "Select a model", options=["gpt-4o-mini", "gpt-4o", "o1-mini", "o1"], index=0
    )
    ...
    st.info("The SERPER API key is needed for the agent to perform web searches.")
